...
export function export_json_to_excel({ header, data, filename, autoWidth = true, bookType = 'xlsx' } = {}) {
  ...
  var wb = new Workbook(),
    ws = sheet_from_array_of_arrays(data),
    merges = [],
    headerNoDup = header.filter(function(item, pos, self) {
      return self.indexOf(item) == pos
    })

  if (header.length > 1) {
    headerNoDup.forEach(item => {
      merges.push(getMergeCell(item, header))
    })
    merges = merges.map(item => ({ s: { c: item.s, r: 0 }, e: { c: item.e, r: 0 } }))
    ws['!merges'] = merges
  }
  if (autoWidth) {
    /*设置worksheet每列的最大宽度*/
